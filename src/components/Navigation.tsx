...
const Navigation = ({ isAdmin, onAdminClick, onLogout }: NavigationProps) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled ? "bg-background/95 backdrop-blur-sm shadow-neon" : "bg-transparent"
    }`}>
        ...
        {isOpen && (
          <div className="md:hidden py-4 border-t border-primary/30">
            ...
            <Button
              size="sm"
              onClick={isAdmin ? onLogout : onAdminClick}
              className={`w-full mt-4 ${isAdmin ? "bg-destructive hover:bg-destructive/90" : "bg-primary hover:bg-primary/90"}`}
            >
              <Shield className="w-4 h-4 mr-2" />
              {isAdmin ? "Logout" : "Admin"}
            </Button>
